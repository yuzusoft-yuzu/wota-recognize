...
# 匹配参数（经实验确定：同视频片段 dHash 距离 2~18、颜色距离 ~0.15；
# 不同视频 dHash 距离 >25 或颜色距离 >0.5）
# 阈值25为甜点：全片/前段精准命中目标，画面相似的不同视频(距离14~23)被排除
DEFAULT_THRESHOLD = 25       # 汉明距离 < 25 视为"同一帧"（转载/同源视频距离通常<15）
